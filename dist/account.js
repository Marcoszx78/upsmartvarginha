(() => {
  ...
  if(form){
    ...
    form.onsubmit=async event=>{event.preventDefault();showError('');const data=Object.fromEntries(new FormData(form));if(mode==='register'&&data.password!==data.confirmation){showError('As senhas não são iguais. Confira e tente novamente.');return;}const button=$('#account-submit');button.disabled=true;button.textContent='Aguarde…';try{const {user}=await api(mode,data);form.reset();if(user.role==='admin'){location.assign('/admin');return;}render(user);}catch(e){showError(e.message);}finally{button.disabled=false;button.textContent=mode==='register'?'Criar minha conta ↗':'Entrar na conta ↗';}};
  }

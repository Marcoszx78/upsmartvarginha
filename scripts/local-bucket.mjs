...
export function localBucket(directory){
 ...
 const file=key=>{if(!/^(avatars|shop)\/[a-zA-Z0-9-]+\.jpg$/.test(key))throw Error('Invalid object key');return path.join(root,key.startsWith('shop/')?'shop-'+path.basename(key):path.basename(key));};
 return {
